...
async def _handle_rl(
    *,
    ...
    if not redis:
        raise Exception("You must call FastAPILimiter.init in the startup event of FastAPI!")
    ...
    # Key format for per route sublimits is fastapi_limiter.sub-METHOD@URL:KEY#index - These are specific to a single route and keep rotating
    # Key format for global ratelimits is fastapi_limiter.global-METHOD@URL:KEY - These are global to a API endpoint and blocks you from the whole API for 3 minutes + milliseconds if you hit it
    # Key format for API wide (ceiling) ratelimit if needed is fastapi_limiter.apiwide:KEY
    # Key format for API block limit (global and API wide trigger this) is fastapi_limiter.block:KEY. It may or may not have a expiry
